...
def get_logger(log_file_name, log_sub_dir=""):
    ...
    handler.setFormatter(CustomFormatter('%(asctime)s  %(levelname)-10s  %(filename)s  %(funcName)s  %(lineno)s  %(message)s'))
    logger.addHandler(handler)
    ch = logging.StreamHandler()
    ch.setLevel(logging.DEBUG)
    ch.setFormatter(CustomFormatter('%(levelname)s:     %(asctime)s    %(filename)s  %(funcName)s  %(lineno)s  %(message)s'))
